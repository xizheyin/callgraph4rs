...
// This example shows how to use dyn traits for dynamic dispatch
...
// Concrete type implementing Signal
struct SineWave {
    frequency: f64,
    amplitude: f64,
}
...
// Use Box<dyn Signal> for dynamic dispatch
...
// Use &dyn Signal> for dynamic dispatch
fn analyze_signal(signal: &dyn Signal, time_points: &[f64]) -> (f64, f64) {
    let mut min_val = f64::INFINITY;
    let mut max_val = f64::NEG_INFINITY;

    for &time in time_points {
        // Dynamic dispatch also happens here
        let sample = signal.sample(time);
        min_val = min_val.min(sample);
        max_val = max_val.max(sample);
    }

    println!("Analyzed signal: {} (range: {} to {})", signal.name(), min_val, max_val);
    (min_val, max_val)
}

// Use Vec<Box<dyn Signal>> to process multiple signals
fn mix_signals(signals: Vec<Box<dyn Signal>>, time: f64) -> f64 {
    let mut result = 0.0;

    for signal in &signals {
        // Dynamic dispatch occurs on each iteration
...
fn share_signal(signal: Rc<dyn Signal>, consumers: usize) -> Vec<f64> {
    ...
    for i in 0..consumers {
        ...
        // Dynamic dispatch through Rc
        let sample = signal.sample(time);
        results.push(sample);
        println!("Consumer {} got sample {} from {}", i, sample, signal.name());
    }

    results
}

// Main function demonstrating several dynamic dispatch scenarios
pub fn main() {
    println!("=== Dynamic Dispatch Example ===");

    // 1. Box<dyn Signal> example
    println!("\n1. Box<dyn Signal> example:");
    ...
    // 3. Vec<Box<dyn Signal>> example
    println!("\n3. Signal mixing example:");
    ...
    // 4. Rc<dyn Signal> example
    println!("\n4. Rc<dyn Signal> sharing example:");
    let shared_signal = Rc::new(SineWave::new(1000.0, 1.0)) as Rc<dyn Signal>;
    let _shared_results = share_signal(shared_signal, 3);

    println!("\n=== Dynamic Dispatch Example Complete ===");
}

// Additional functional-style example
fn functional_style_example() {
    println!("\n=== Functional-Style Dynamic Dispatch ===");
    ...
    // Use closures together with dynamic dispatch
    let time = 0.001;
    let samples: Vec<f64> = signals
        .iter()
        .map(|signal| signal.sample(time)) // Dynamic dispatch happens here
        .collect();

    println!("Samples at t={}: {:?}", time, samples);

    // Find the signal with the maximum amplitude
    let max_signal = signals.iter().max_by(|a, b| {
        let a_sample = a.sample(time).abs(); // Dynamic dispatch
        let b_sample = b.sample(time).abs(); // Dynamic dispatch
