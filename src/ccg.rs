...
impl<'tcx> Dependencies<'tcx> {
    ...
    fn direct_dependencies<'mir>(function: &'mir mir::Body<'tcx>) -> Self {
        ...
        let constants: Vec<Const<'_>> = extract_constant(function).into_iter().collect();
                    ...
                    match op {
                        ...
                        Constant(constant) => {
                            locals_count
                                + constants
                                    .iter()
                                    .position(|cst| cst == &constant.const_)
                                    .unwrap()
                        }
                    }
                };
...
struct CallSite<'tcx> {
    ...
    arguments: Vec<Spanned<mir::Operand<'tcx>>>,
}
...
fn extract_constant<'tcx>(function: &mir::Body<'tcx>) -> HashSet<Const<'tcx>> {
    use mir::visit::Visitor;

    #[derive(Default)]
    struct Constants<'tcx> {
        constants: HashSet<Const<'tcx>>,
    }
    impl<'tcx> Visitor<'tcx> for Constants<'tcx> {
        fn visit_operand(&mut self, operand: &mir::Operand<'tcx>, _: mir::Location) {
            if let mir::Operand::Constant(constant) = operand {
                self.constants.insert(constant.const_);
            }
        }
    }
        ...
        fn visit_terminator(&mut self, terminator: &Terminator<'tcx>, _location: mir::Location) {
            ...
            } = &terminator.kind
            {
                ...
                self.callees.push(CallSite {
                    return_variable: destination.as_local(),
                    function,
...
pub fn extract_dependencies<'tcx>(tcx: ty::TyCtxt<'tcx>) -> AllDependencies<'tcx> {
    ...
    for function in tcx.hir().body_owners() {
        match tcx.def_kind(function) {
            def::DefKind::Fn | def::DefKind::AssocFn | def::DefKind::Closure => (),
            _ => continue,
        }
        ...
        for callsite in &callsites {
            ...
            for arg in &callsite.arguments {
                ...
                match arg {
                    ...
                    Constant(cst) => {
                        if is_callable(&cst.const_.ty()) {
                            sources.push(Source::FunctionId(cst.const_));
                        }
                    }
                }
            }
...
pub fn render_dependencies<'tcx, W: std::io::Write>(
    ...
    for (caller, function) in &all_dependencies.functions {
        if tcx.def_kind(*caller) == DefKind::Closure {
            continue;
        }
        ...
        writeln!(output, "    \"{}\" [ label=<<table border=\"0\" cellpadding=\"2\" cellspacing=\"0\" cellborder=\"0\"><tr>", escaped_caller_name)?;
        writeln!(
            output,
        ...
        for (arg_id, SymbolAndType { symbol, ty }) in function.arguments.iter().enumerate() {
            ...
            let ty: ty::GenericArg<'_> = (*ty).into();
            let ty = format!("{}", ty);
        ...
        if !function.return_ty.is_unit() {
            let ty: ty::GenericArg<'_> = function.return_ty.into();
            let ty = format!("{}", ty);
            let ty = html_escape::encode_text(&ty);
            let right_arrow = "&#8594;";
                ...
                match source {
                    FunctionId(source) => {
                        if !indirect_dependencies.contains(&(source, caller)) {
                            ...
                            let source_name = print_const(source, tcx);
                            writeln!(
                                output,
                    ...
                    for source in sources {
                        ...
                        match source {
                            FunctionId(callee) => {
                                ...
                                let style =
                                    if internal_functions.contains(&get_const_def_id(callee)) {
                                        "solid"
                                    } else {
                                        "dotted"
                                    };
...
pub fn print_const(const_val: &Const<'_>, tcx: ty::TyCtxt<'_>) -> String {
    match const_val.ty().kind() {
        ty::TyKind::FnDef(def_id, _) => tcx.def_path_str(*def_id),
        _ => String::new(),
    }
}
pub fn get_const_def_id(const_val: &Const<'_>) -> DefId {
    match const_val.ty().kind() {
...
// 添加一个新的函数，用于将依赖图输出到 target 目录
pub fn output_dependencies_to_target<'tcx>(
    tcx: ty::TyCtxt<'tcx>,
    all_dependencies: AllDependencies<'tcx>,
) -> std::io::Result<()> {
    // 确保 target 目录存在
    ...
    // 创建输出文件
    let output_path = "target/dependencies.dot";
    let mut file = std::fs::File::create(output_path)?;

    // 渲染依赖关系到文件
    render_dependencies(tcx, all_dependencies, &mut file)?;

    // 获取绝对路径并打印更详细的信息
    if let Ok(current_dir) = std::env::current_dir() {
        let absolute_path = current_dir.join(output_path);
        eprintln!("Dependencies graph written to: {}", absolute_path.display());
    } else {
        eprintln!("Dependencies graph written to: {}", output_path);
    }

    // 尝试使用 dot 命令渲染生成的图表
    let png_path = "target/dependencies.png";
    if let Ok(status) = std::process::Command::new("dot")
        .args(["-Tpng", output_path, "-o", png_path])
        .status()
    {
        if status.success() {
            if let Ok(current_dir) = std::env::current_dir() {
                let absolute_png_path = current_dir.join(png_path);
                eprintln!("PNG image generated at: {}", absolute_png_path.display());
            } else {
                eprintln!("PNG image generated at: {}", png_path);
            }
        }
    }
